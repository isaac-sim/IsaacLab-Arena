...
class place_gear_in_gripper(ManagerTermBase):
    ...
    def __call__(
        ...
        for _ in range(max_ik_iterations):
            ...
            target_pos = gear_pos_w + math_utils.quat_apply(target_quat, grasp_offset_batch)
        ...
        if not hasattr(env, "_initial_gear_ee_relative_quat"):
            env._initial_gear_ee_relative_quat = torch.zeros(env.num_envs, 4, device=env.device, dtype=torch.float32)
