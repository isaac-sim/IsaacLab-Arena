...
"""ArenaEnvInitialGraphSpec review tool — Streamlit live editor + live USD thumbnails.
...
def render_html_for_spec(spec: ArenaEnvInitialGraphSpec, thumbnails: dict[str, bytes] | None = None) -> str:
...
def load_cached_thumbnails(spec: ArenaEnvInitialGraphSpec) -> dict[str, bytes]:
...
class SimAppSidecar:
    ...
    def render_spec(self, spec: ArenaEnvInitialGraphSpec) -> dict[str, bytes]:
        ...
        Serializes the spec back to YAML (via ``ArenaEnvInitialGraphSpec.to_dict``
        ...
        trust ``ArenaEnvInitialGraphSpec.from_dict`` to be the canonical parser.
...
def _render_html(spec: ArenaEnvInitialGraphSpec, thumbnails: dict[str, bytes] | None = None) -> str:
...
def _render_node_cards(spec: ArenaEnvInitialGraphSpec, thumbnails: dict[str, bytes]) -> str:
