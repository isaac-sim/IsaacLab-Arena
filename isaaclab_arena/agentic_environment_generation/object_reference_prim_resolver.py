...
def prim_tree_catalog(prim_tree: list[UsdPrimRecord]) -> str:
...
def object_reference_context(spec: ArenaEnvGraphSpec) -> str:
...
def build_resolve_user_message(spec: ArenaEnvGraphSpec, prim_tree: list[UsdPrimRecord]) -> str:
    ...
    return f"{prim_tree_catalog(prim_tree)}\n\n{object_reference_context(spec)}"
...
class ObjectReferencePrimResolver:
    ...
    def __init__(self, query_backend: QueryBackend):
        ...
        self._schema = build_strict_schema(ResolveObjectReferencePrimPathsResult)
    ...
    def infer(
        ...
        result = self._query_backend.run_json(
            StructuredOutputRequest(
                schema_name="ResolveObjectReferencePrimPathsResult",
                schema=self._schema,
                system=resolve_system_prompt(),
                user=build_resolve_user_message(spec, prim_tree),
                retry_label="resolve_usd_prim",
            )
        )
        resolved = ResolveObjectReferencePrimPathsResult.model_validate(result.data).object_references
        spec = merge_resolved_object_references(spec, resolved)
