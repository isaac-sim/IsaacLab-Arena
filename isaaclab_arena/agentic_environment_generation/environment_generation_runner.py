...
Pipeline:
  ...
  2. :class:`~isaaclab_arena.agentic_environment_generation.intent_compiler.IntentCompiler` →
     :class:`~isaaclab_arena.environments.arena_env_graph_spec.UnresolvedArenaEnvGraphSpec`.
  3. :meth:`UnresolvedArenaEnvGraphSpec.resolve` →
     :class:`~isaaclab_arena.environments.arena_env_graph_spec.ArenaEnvGraphSpec`.
  4. Both specs are written to ``--out-dir`` as YAML files. The resolved spec is
...
Usage::
    ...
    /isaac-sim/python.sh -m isaaclab_arena.agentic_environment_generation.environment_generation_runner \\
        --headless --num_envs 1 \\
...
def main() -> int:
    ...
    with SimulationAppContext(args_cli):
        ...
        from isaaclab_arena.environments.arena_env_graph_spec import ArenaEnvGraphSpec, UnresolvedArenaEnvGraphSpec
        ...
        # ------------------------------------------------------------------ #
        # Step 2 — EnvironmentIntentSpec → UnresolvedArenaEnvGraphSpec       #
        # ------------------------------------------------------------------ #
        compiler = IntentCompiler()
        unresolved_spec: UnresolvedArenaEnvGraphSpec = compiler.compile(intent_spec)

        print(
            f"[runner] compiled → {len(unresolved_spec.nodes)} nodes, "
            f"{len(unresolved_spec.tasks)} tasks, "
            f"env_name={unresolved_spec.env_name!r}",
            flush=True,
        )
        ...
        # ------------------------------------------------------------------ #
        # Step 3 — UnresolvedArenaEnvGraphSpec.resolve() → ArenaEnvGraphSpec #
        # ------------------------------------------------------------------ #
        resolved_spec: ArenaEnvGraphSpec = unresolved_spec.resolve()
        print(
            f"[runner] resolved → {len(resolved_spec.state_specs)} state specs, {len(resolved_spec.tasks)} wired tasks",
            flush=True,
        )

        # ------------------------------------------------------------------ #
        # Step 4 — dump both specs to YAML, then reload the resolved one     #
        # ------------------------------------------------------------------ #
        out_dir: Path = args_cli.out_dir
        out_dir.mkdir(parents=True, exist_ok=True)
        stem = _safe_stem(unresolved_spec.env_name)

        unresolved_path = out_dir / f"{stem}_unresolved.yaml"
        resolved_path = out_dir / f"{stem}_resolved.yaml"

        unresolved_spec.write_yaml(unresolved_path)
        print(f"[runner] wrote unresolved spec → {unresolved_path}", flush=True)

        resolved_spec.write_yaml(resolved_path)
        print(f"[runner] wrote resolved spec   → {resolved_path}", flush=True)

        # Reload from disk to verify the YAML round-trip before building the env.
        loaded_spec = ArenaEnvGraphSpec.from_yaml(resolved_path)
        print(f"[runner] reloaded resolved spec from {resolved_path}", flush=True)
