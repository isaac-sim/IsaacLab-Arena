...
# Matches the per-episode results filename written by EpisodeRecorderManager.write. The Arena Experiment Runner
...
def _scan_jobs(root: pathlib.Path) -> list[JobReport]:
    ...
    - The arena_experiment_runner.py writes one per-job sub-directory under ``root``.
