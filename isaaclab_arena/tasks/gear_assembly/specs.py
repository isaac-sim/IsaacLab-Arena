...
def _newton_tabletop_parking_positions() -> dict[str, tuple[float, float, float]]:
    return {
        "gear_small": (
...
NEWTON_GEAR_TABLETOP_PARKING_POSITIONS = _newton_tabletop_parking_positions()
...
NEWTON_GEAR_ASSEMBLED_ROOT_Z_ABOVE_BASE = {gear_type: 0.0075 for gear_type in GEAR_TYPES}
GEAR_ASSEMBLED_XY_THRESHOLD = 0.015
GEAR_ASSEMBLED_Z_THRESHOLD = 0.01
GEAR_ASSEMBLED_UPRIGHT_AXIS_THRESHOLD_DEG = 15.0
GEAR_ASSEMBLED_LINEAR_VELOCITY_THRESHOLD = 0.05
GEAR_ASSEMBLED_ANGULAR_VELOCITY_THRESHOLD = 0.5
GEAR_ASSEMBLED_SUPPORT_Z_OFFSET = {
    "gear_small": -0.017,
    "gear_medium": 0.0,
    "gear_large": -0.003,
}
NEWTON_GEAR_ASSEMBLED_SUPPORT_Z_OFFSET = {gear_type: 0.0 for gear_type in GEAR_TYPES}
GEAR_ASSEMBLED_SUPPORT_Z_THRESHOLD = 0.005
GEAR_ASSEMBLED_CONSECUTIVE_SUCCESS_STEPS = 10
...
def get_droid_robot_spec(newton_backend: bool = False) -> GearAssemblyRobotSpec:
    ...
    if newton_backend:
        ...
        gripper_joint_setter_func = _set_newton_droid_gripper_joint_pos
        robot_material = (2.0, 2.0, 0.0)
...
def _set_newton_droid_gripper_joint_pos(
    joint_pos: torch.Tensor,
    reset_ind_joint_pos: list[int],
    gripper_joints: list[int],
    joint_position: float,
) -> None:
    """Set all explicit Robotiq mimic joints for Newton."""
    assert len(gripper_joints) >= 6, f"Droid gripper requires at least 6 gripper joints, got {len(gripper_joints)}"
