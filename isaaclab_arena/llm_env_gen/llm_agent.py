...
class LLMAgent:
    ...
    def _system_prompt(self) -> str:
        ...
        return (
            ...
            "    cabinet). Their target MUST be null. Use them to express the state of the appliance:\n"
            "      initial 'closed(microwave)' + final 'open(microwave)'  → task is to open the door\n"
            "      initial 'open(microwave)'   + final 'closed(microwave)' → task is to close the door\n"
            "  * Articulated objects (microwave etc.) still need a regular 'on(microwave, background)'\n"
            "    in the initial graph so the placer knows they sit on the counter. The 'open'/'closed'\n"
            "    state marker is IN ADDITION to the spatial 'on' relation.\n"
            "  * Distractor items around the appliance also need 'on(distractor, background)' relations.\n"
            ...
            "  For 'open microwave': initial has `closed(microwave, null)` + `on(microwave, background)`,\n"
            "  final has `open(microwave, null)` + `on(microwave, background)` (position unchanged).\n"
