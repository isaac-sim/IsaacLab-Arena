...
def _test_sequential_success_advances_in_order(simulation_app) -> bool:
    ...
    try:
        ...
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, None)
...
def _test_sequential_success_latches(simulation_app) -> bool:
    ...
    try:
        ...
        subtasks[0].set_success([True])
        SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, None)
        subtasks[1].set_success([True])
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, None)
        assert result.tolist() == [True]
...
def _test_sequential_desired_subtask_success_state(simulation_app) -> bool:
    ...
    try:
        ...
        subtasks[0].set_success([True])
        SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [True, True])
        subtasks[1].set_success([True])

        # Current pattern matches desired -> success.
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [True, True])
        assert result.tolist() == [True]
...
def _test_sequential_desired_subtask_success_state_with_none(simulation_app) -> bool:
    ...
    try:
        ...
        subtasks[0].set_success([True])
        SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [None, True, True])
        subtasks[1].set_success([True])
        SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [None, True, True])
        subtasks[2].set_success([True])
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [None, True, True])
        assert env._subtask_ever_succeeded == [[True, True, True]]
        assert result.tolist() == [True]
        ...
        # subtasks 0 and 2.
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [None, False, None])
        assert result.tolist() == [True]

        # All-None desired state matches trivially.
        result = SequentialTaskBase.composite_task_success_func(env, resolved_subtask_success_cfgs, [None, None, None])
        assert result.tolist() == [True]
