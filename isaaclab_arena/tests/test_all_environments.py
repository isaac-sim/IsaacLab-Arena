...
"""Smoke-test every registered environment via arena_experiment_runner.

Auto-discovers environments from the EnvironmentRegistry and runs each one for
a few steps with the zero_action policy in a single arena_experiment_runner subprocess.
...
from isaaclab_arena.tests.test_arena_experiment_runner import run_arena_experiment_runner, write_jobs_config_to_file
...
def test_arena_experiment_runner_all_environments(tmp_path):
    ...
    config_path = str(tmp_path / "test_arena_experiment_runner_all_environments.json")
    write_jobs_config_to_file(jobs, config_path)
    run_arena_experiment_runner(config_path, headless=HEADLESS)
