...
def test_pooled_placer_falls_back_when_no_valid_layouts(capsys):
    ...
    desk = DummyObject(
        name="desk",
        bounding_box=OrientedBoundingBox.from_min_max(min_point=(0.0, 0.0, 0.0), max_point=(0.01, 0.01, 0.01)),
    )
    desk.set_initial_pose(Pose(position_xyz=(0.0, 0.0, 0.0), rotation_xyzw=(0.0, 0.0, 0.0, 1.0)))
    desk.add_relation(IsAnchor())
    ...
    big1 = DummyObject(
        name="big1",
        bounding_box=OrientedBoundingBox.from_min_max(min_point=(0.0, 0.0, 0.0), max_point=(5.0, 5.0, 5.0)),
    )
    big2 = DummyObject(
        name="big2",
        bounding_box=OrientedBoundingBox.from_min_max(min_point=(0.0, 0.0, 0.0), max_point=(5.0, 5.0, 5.0)),
...
class _StubReachabilityValidator(PlacementValidator):
    ...
    def validate_batch(self, positions, rotations, bboxes, collision_objects):
        from isaaclab_arena.relations.placement_result import PlacementResult
        from isaaclab_arena.relations.placement_validation import PlacementValidationResults
