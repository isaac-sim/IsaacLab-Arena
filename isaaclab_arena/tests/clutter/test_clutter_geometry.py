...
def test_resting_containment_uses_rotated_bounds():
    ...
    region = region_above_support((0, 0, 1), support, rotation)
    with pytest.raises(AssertionError, match="Only 90°"):
        region_above_support((0, 0, 1), support, (0, 0, math.sin(math.pi / 8), math.cos(math.pi / 8)))
    ...
    verdict = check_resting_poses(bounds, region, ClutterSettleParams())
