...
def test_spread_shrinks_the_region_about_its_center():
...
def test_off_center_support_region_follows_its_rotation():
...
def test_support_with_no_pose_anywhere_is_rejected():
    ...
    support = _Asset("table")
    support._pose = None
    with pytest.raises(AssertionError, match="needs a solved position or a fixed Pose"):
...
def test_support_pose_that_stands_for_several_is_rejected():
    ...
    for declared in (
        ...
        with pytest.raises(AssertionError, match="fixed Pose"):
            support_pose_from_layout(support, _Layout())
...
def test_reference_region_does_not_rotate_parent_frame_bounds_twice():
    from types import SimpleNamespace

    from isaaclab_arena.assets.object_reference import ObjectReference
    from isaaclab_arena.relations.clutter_pour import region_for_support

    parent_pose = Pose(rotation_xyzw=(0.0, 0.0, math.sqrt(0.5), math.sqrt(0.5)))
    parent = SimpleNamespace(initial_pose=parent_pose, get_initial_pose=lambda: parent_pose)
    support = ObjectReference.__new__(ObjectReference)
    support.name = "bin_floor"
    support.parent_asset = parent
    support.initial_pose_relative_to_parent = Pose(rotation_xyzw=(0.0, 0.0, math.sqrt(0.5), math.sqrt(0.5)))
    support.relations = [IsAnchor()]
    region = region_for_support(support, _Layout(), {support: _box(0.4, 0.2, 0.05)})
    assert region.max_x == pytest.approx(0.2)
    assert region.max_y == pytest.approx(0.4)
