...
def _test_galbot_arm_reaches_goal(simulation_app) -> bool:
    ...
    try:
        with torch.inference_mode():
            ...
            initial_ee_pos = wp.to_torch(ee_frame.data.target_pos_w)[0, 0, :] - env.scene.env_origins[0]
            ...
            for step in range(num_reach_steps):
                ...
                current_ee_pos = wp.to_torch(ee_frame.data.target_pos_w)[0, 0, :] - env.scene.env_origins[0]
            ...
            final_ee_pos = wp.to_torch(ee_frame.data.target_pos_w)[0, 0, :] - env.scene.env_origins[0]
