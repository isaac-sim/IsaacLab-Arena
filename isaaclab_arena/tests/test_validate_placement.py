...
@pytest.mark.parametrize("marker_yaw", [0.0, math.pi / 4, math.pi / 2, -math.pi / 2])
def test_marker_only_rotation_matches_applied_pose(marker_yaw):
    """Marker-only rotations reach solver bounds, validation, and the applied pose once."""
    ...
    anchor.set_initial_pose(Pose(position_xyz=(10.0, 0.0, 0.0)))
    subject = _make_long_box("subject")
    subject.add_relation(RotateAroundSolution(yaw_rad=marker_yaw))
    obstacle = _make_box("obstacle", size=0.1)
    positions = {anchor: (10.0, 0.0, 0.0), subject: (0.0, 0.0, 0.0), obstacle: (0.0, 0.15, 0.0)}
    placer = ObjectPlacer(ObjectPlacerParams(random_yaw_init=False))
    rotations = placer._generate_initial_rotations(list(positions), {anchor})
    state = RelationSolverState(list(positions), [positions], rotations=[rotations])
    solver_box = state.get_bbox(subject).translated(state.get_position(subject))
    placer._apply_poses([positions], {anchor}, [rotations])
    applied_box = subject.get_world_bounding_box()
    torch.testing.assert_close(solver_box.get_corners(), applied_box.get_corners())
    c, s = abs(math.cos(marker_yaw)), abs(math.sin(marker_yaw))
    minimum, maximum = solver_box.get_axis_aligned_bounds()
    torch.testing.assert_close((maximum - minimum) / 2, torch.tensor([[0.3 * c + 0.05 * s, 0.3 * s + 0.05 * c, 0.05]]))
    validation = _validate_one(placer, positions, _env_bboxes(positions), rotations)
    assert validation.do_all_required_validation_checks_pass() == (marker_yaw == 0.0)
    result = PlacementResult(
        positions=positions, rotations=rotations, validation_results=validation, final_loss=0, attempts=1
    )
    assert get_pose_from_layout(subject, result) == subject.get_initial_pose()
