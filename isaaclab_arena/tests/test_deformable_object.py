...
"""Focused configuration and Newton smoke tests for volume deformables."""
...
def _test_volume_deformable_config(simulation_app) -> bool:
    ...
    with pytest.raises(AssertionError, match="does not support volume deformables"):
        soft_cube.resolve_object_cfg("physx")
    return True


def _test_newton_deformable_smoke(simulation_app) -> bool:
    import torch
...
def test_newton_deformable_smoke():
    assert run_function_with_persistent_simulation_app(_test_newton_deformable_smoke, headless=HEADLESS)
