...
def test_no_collision_loss_scales_with_slope():
    ...
    assert torch.isclose(loss_20[0], 2.0 * loss_10[0], rtol=1e-5)
...
def test_no_collision_loss_scales_with_relation_weight():
    ...
    assert torch.isclose(loss_2[0], 2.0 * loss_1[0], rtol=1e-5)


def test_no_collision_loss_batched_shape_and_values():
    """Test that NoCollision with batched input has loss shape (N,) and correct per-env values."""
    box_a = _create_box("box_a")
    box_b = _create_box("box_b")
    relation = NoCollision(box_b, clearance_m=0.0)
    strategy = NoCollisionLossStrategy(slope=10.0)
...
def test_no_collision_loss_volume_formula():
    ...
    loss = strategy.compute_loss(relation, child_pos, box_a.bounding_box, parent_world_bbox)
    assert torch.isclose(loss[0], torch.tensor(expected_loss), rtol=1e-4)
...
def test_relation_solver_no_collision_produces_separated_positions():
    ...
    pos_a = result[0][box_a]
    pos_b = result[0][box_b]
...
def test_relation_solver_no_collision_same_inputs_reproducible():
    ...
    assert result1[0][box_a1] == result2[0][box_a2], "box_a positions should match"
    assert result1[0][box_b1] == result2[0][box_b2], "box_b positions should match"
