...
from isaaclab_arena.agentic_environment_generation.state_spec_resolver import StateSpecResolver
...
def _spatial_by_id(state: ArenaEnvGraphStateSpec) -> dict[str, tuple]:
    ...
    return {c.id: (c.type.value, c.parent, c.child, c.params) for c in state.spatial_constraints}
...
def test_populate_reproduces_golden_full_graph():
    """Resolving the partial init graph yields the hand-authored full graph (structurally)."""
    populated = StateSpecResolver.from_yaml(_INIT_GRAPH).resolve_path()
    golden = ArenaEnvGraphSpec.from_yaml(_FULL_GRAPH)

    # Same set of states, chained 0..N.
    assert set(populated.state_specs_by_id) == set(golden.state_specs_by_id)

    # Each state's spatial + task constraints match (keyed by id, so order is irrelevant).
    for state_id, golden_state in golden.state_specs_by_id.items():
        got = populated.state_specs_by_id[state_id]
        assert _spatial_by_id(got) == _spatial_by_id(golden_state), f"spatial mismatch in {state_id}"
        assert _task_constraints_by_id(got) == _task_constraints_by_id(golden_state), f"task mismatch in {state_id}"

    # Tasks are wired into the chain identically.
    assert set(populated.tasks_by_id) == set(golden.tasks_by_id)
    for task_id, golden_task in golden.tasks_by_id.items():
        got = populated.tasks_by_id[task_id]
        assert got.type == golden_task.type
        assert got.initial_state_spec_id == golden_task.initial_state_spec_id
        assert got.success_state_spec_id == golden_task.success_state_spec_id
        assert got.task_args == golden_task.task_args
...
def test_task_without_a_transition_is_rejected():
    ...
    resolver = StateSpecResolver.from_yaml(_INIT_GRAPH)
    resolver.graph.tasks[0].type = "NoTask"  # registered, but declares no transition
