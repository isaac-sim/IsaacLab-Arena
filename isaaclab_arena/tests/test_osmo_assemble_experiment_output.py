...
from osmo.scripts.assemble_experiment_output import (
    assemble_experiment_output,
    load_staged_experiment_output_directories_by_run_name,
)
...
def test_loads_staged_experiment_output_directories_as_paths(tmp_path):
    serialized_output_directories_path = tmp_path / "staged-experiment-output-directories.json"
    staged_experiment_output_directory = tmp_path / "experiment-runner-0-output"
    serialized_output_directories_path.write_text(
        json.dumps({"first": str(staged_experiment_output_directory)}),
        encoding="utf-8",
    )
    ...
    assert staged_experiment_output_directories_by_run_name == {"first": staged_experiment_output_directory}


def test_rejects_staged_experiment_output_without_the_requested_run(tmp_path):
    staged_experiment_output_directory = tmp_path / "experiment-runner-0-output"
    (staged_experiment_output_directory / "another-run").mkdir(parents=True)

    with pytest.raises(AssertionError, match="Expected Run output directory for Run 'first'.*first"):
        assemble_experiment_output(
            {"first": staged_experiment_output_directory},
            tmp_path / "experiment-output",
        )


def test_assembles_experiment_output_from_separate_staged_experiment_outputs(tmp_path):
    first_staged_experiment_output_directory = tmp_path / "experiment-runner-0-output"
    second_staged_experiment_output_directory = tmp_path / "experiment-runner-1-output"
    first_run_output_directory = first_staged_experiment_output_directory / "first"
    second_run_output_directory = second_staged_experiment_output_directory / "second"
    ...
    report_path = assemble_experiment_output(
        {
            "first": first_staged_experiment_output_directory,
            "second": second_staged_experiment_output_directory,
        },
