...
def _patch_bounding_box_helpers_for_test_doubles(monkeypatch):
    ...
    def has_het_with_doubles(objects):
        return original_has_het(objects) or any(isinstance(obj, HeterogeneousDummyObject) for obj in objects)

    def bbox_per_env_with_doubles(obj, num_envs):
        if isinstance(obj, HeterogeneousDummyObject):
...
def test_rotated_variants_keep_env_identity_across_unequal_layout_counts():
    import math
    ...
    desk = _make_desk()
    boxes = [
        OrientedBoundingBox.from_min_max((0.05, -0.1, 0.0), (0.25, 0.0, 0.1)),
        OrientedBoundingBox.from_min_max((-0.1, 0.0, 0.0), (0.2, 0.1, 0.15)),
    ]
    obj = HeterogeneousDummyObject("variants", boxes)
    obj.add_relation(On(desk, clearance_m=0.01))
    obj.add_relation(RotateAroundSolution(roll_rad=0.2, pitch_rad=0.4, yaw_rad=math.pi / 3))
    params = ObjectPlacerParams(
        max_placement_attempts=2,
        placement_seed=17,
        solver_params=RelationSolverParams(max_iters=0, verbose=False),
        apply_positions_to_objects=False,
        allow_best_loss_fallbacks=False,
    )
    pool = PooledObjectPlacer([desk, obj], params, pool_size=5, num_envs=2)
    assert [len(layouts) for layouts in pool.layouts_per_env()] == [3, 3]
    selected = pool.sample_for_envs([1])
    assert [env_pool.cursor for env_pool in pool._env_pools] == [0, 1]
    for env_id, layouts in enumerate(pool.layouts_per_env()):
        for layout in layouts:
            assert layout.success
            world_box = boxes[env_id].transformed(layout.positions[obj], layout.rotations[obj])
            minimum, _ = world_box.get_axis_aligned_bounds()
            assert minimum[0, 2].item() == pytest.approx(0.11, abs=1e-6)
    assert selected[1] is pool.layouts_per_env()[1][0]
