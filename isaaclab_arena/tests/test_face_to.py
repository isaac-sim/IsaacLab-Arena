...
@pytest.mark.parametrize(
    "rotation, expected",
    [
        ((0.0, 0.0, 2**-0.5, 2**-0.5), True),
        ((0.0, 0.0, -(2**-0.5), -(2**-0.5)), True),
        ((0.0, 0.0, 0.0, 1.0), False),
        ((0.0, 0.0, 0.0, 2.0), False),
    ],
)
def test_face_to_validator_checks_rotation_value(rotation, expected):
    from isaaclab_arena.relations.placement_validators import FaceToValidator

    pair = _face_to_pair(target_position=(0.0, 2.0, 0.0))
    positions = {pair.target: (0.0, 2.0, 0.0), pair.subject: (0.0, 0.0, 0.0)}
    validator = FaceToValidator(ObjectPlacerParams())
    assert validator.validate_batch([positions], [{pair.subject: rotation}], [{}], []) == [expected]
