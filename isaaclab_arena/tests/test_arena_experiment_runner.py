...
def test_arena_experiment_runner_parses_native_hydra_overrides():
...
def test_arena_experiment_runner_rejects_unknown_non_hydra_arguments():
    """Reject misspelled CLI flags in a fresh process."""
    result = subprocess.run(
        [TestConstants.python_path, f"{TestConstants.evaluation_dir}/arena_experiment_runner.py", "--headles"],
        capture_output=True,
        text=True,
        check=False,
        timeout=60,
...
def run_arena_experiment_runner(
    ...
    """Run the Arena Experiment Runner as a subprocess with timeout.

    --continue_on_error is NOT passed, so the Arena Experiment Runner re-raises on the
    ...
    Args:
        ...
        extra_args: Additional Arena Experiment Runner arguments.
    ...
    """
    args = [TestConstants.python_path, f"{TestConstants.evaluation_dir}/arena_experiment_runner.py"]
...
@pytest.mark.with_subprocess
def test_arena_experiment_runner_from_typed_yaml(tmp_path):
    """Execute a typed YAML Experiment through the Arena Experiment Runner CLI."""
...
""",
    ...
    result = run_arena_experiment_runner(
...
@pytest.mark.with_subprocess
def test_arena_experiment_runner_two_jobs_zero_action(tmp_path):
    """Test arena_experiment_runner with 2 jobs using zero_action policy on different objects."""
    jobs = [
        {
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_two_jobs_zero_action.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_multiple_environments(tmp_path):
    """Test arena_experiment_runner with jobs across different environments."""
    jobs = [
        {
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_multiple_environments.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_different_embodiments(tmp_path):
    """Test arena_experiment_runner with jobs using different embodiments."""
    jobs = [
        {
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_different_embodiments.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_from_existing_config():
    """Test arena_experiment_runner using the zero_action_jobs_config.json and verify no jobs failed."""
    config_path = f"{TestConstants.arena_environments_dir}/eval_jobs_configs/zero_action_jobs_config.json"
    assert os.path.exists(config_path), f"Config file not found: {config_path}"
    run_arena_experiment_runner(config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_with_variations(tmp_path):
    """Test arena_experiment_runner applies a per-job variations block via Hydra overrides."""
    jobs = [
        {
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_with_variations.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_enable_cameras(tmp_path):
    """Test arena_experiment_runner with enable_cameras set to true."""
    jobs = [
        {
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_enable_cameras.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)


@pytest.mark.with_subprocess
def test_arena_experiment_runner_graph_spec_with_variation(tmp_path):
    ...
    YAML, exercising that the Arena Experiment Runner builds graph-spec envs and that --enable_cameras reaches
    ...
    temp_config_path = str(tmp_path / "test_arena_experiment_runner_graph_spec_with_variation.json")
    write_jobs_config_to_file(jobs, temp_config_path)
    run_arena_experiment_runner(temp_config_path)
