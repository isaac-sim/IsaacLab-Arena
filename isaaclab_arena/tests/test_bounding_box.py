...
def test_broadphase_and_axis_aligned_pairs_skip_sat(monkeypatch):
    """Only overlapping rotated pairs reach the expensive SAT projections."""
    original = OrientedBoundingBox._sat_penetration
    counts = []

    def record_sat(center_delta, *args):
        counts.append(center_delta.shape[0])
        return original(center_delta, *args)

    monkeypatch.setattr(OrientedBoundingBox, "_sat_penetration", staticmethod(record_sat))
    centers = torch.tensor([[0.2, 0.0, 0.0], [0.2, 0.0, 0.0], [20.0, 0.0, 0.0]], requires_grad=True)
    boxes = OrientedBoundingBox(centers, (1.0, 0.5, 0.3), torch.tensor([IDENTITY, _yaw(0.4), _pitch(0.7)]))
    obstacle = OrientedBoundingBox((0, 0, 0), (1.0, 0.5, 0.3), IDENTITY)
    depths = boxes.penetration(obstacle)
    depths.sum().backward()
    assert counts == [1]
    assert depths[:2].gt(0).all() and depths[2] == 0
    torch.testing.assert_close(centers.grad[2], torch.zeros(3))

    counts.clear()
    obstacle.penetration(obstacle)
    assert counts == []


@pytest.mark.parametrize("rotation", [IDENTITY, _yaw(0.37)])
def test_all_culled_pairs_have_zero_gradients(rotation):
    """A fully separated batch still permits backward with zero position gradients."""
    center = torch.tensor([100.0, 0.0, 0.0], requires_grad=True)
    box = OrientedBoundingBox(center, (1.0, 0.5, 0.3), rotation)
    other = OrientedBoundingBox((0, 0, 0), (1.0, 0.5, 0.3), IDENTITY)
    box.penetration(other).sum().backward()
    torch.testing.assert_close(center.grad, torch.zeros(3))


def test_seeded_sampling_preserves_global_rng():
    """Sampling with a seed does not change subsequent unrelated random draws."""
    state = torch.random.get_rng_state()
    box = OrientedBoundingBox((0, 0, 0), (1, 1, 1), IDENTITY)
    get_random_pose_within_bounding_box(box, seed=42)
    torch.testing.assert_close(torch.random.get_rng_state(), state)


@pytest.mark.parametrize("value", [float("inf"), float("nan")])
def test_nonfinite_min_max_rejected(value):
    with pytest.raises(AssertionError, match="finite"):
        OrientedBoundingBox.from_min_max((0, 0, 0), (value, 1, 1))


@pytest.mark.parametrize("device", ["cpu", "cuda"])
def test_sat_matches_independent_corner_projection_oracle(device):
    """Full 3D SAT agrees with projected corner intervals, including containment."""
    import itertools
    import numpy as np
    ...
    count = 64
    centers_a = rng.uniform(-0.6, 0.6, (count, 3))
    centers_b = rng.uniform(-1.5, 1.5, (count, 3))
    extents_a = rng.uniform(0.1, 1.5, (count, 3))
    extents_b = rng.uniform(0.1, 1.5, (count, 3))
    rotations_a = Rotation.random(count, random_state=rng)
    rotations_b = Rotation.random(count, random_state=rng)
    signs = np.array(list(itertools.product((-1, 1), repeat=3)))
    expected = []
    for i in range(count):
        vertices_a = rotations_a[i].apply(signs * extents_a[i]) + centers_a[i]
        vertices_b = rotations_b[i].apply(signs * extents_b[i]) + centers_b[i]
        axes_a, axes_b = rotations_a[i].as_matrix().T, rotations_b[i].as_matrix().T
        axes = [*axes_a, *axes_b, *(np.cross(a, b) for a in axes_a for b in axes_b)]
        depths = []
        for axis in axes:
            if np.linalg.norm(axis) < 1e-6:
                continue
            axis = axis / np.linalg.norm(axis)
            a, b = vertices_a @ axis, vertices_b @ axis
            depths.append(min(a.max() - b.min(), b.max() - a.min()) + 0.03)
        expected.append(max(0.0, min(depths)))
    assert any(depth > 0 for depth in expected) and any(depth == 0 for depth in expected)
    boxes_a = OrientedBoundingBox(
        *(torch.tensor(v, device=device) for v in (centers_a, extents_a, rotations_a.as_quat()))
    )
    boxes_b = OrientedBoundingBox(
        *(torch.tensor(v, device=device) for v in (centers_b, extents_b, rotations_b.as_quat()))
    )
    torch.testing.assert_close(
        boxes_a.penetration(boxes_b, 0.03).cpu(), torch.tensor(expected, dtype=torch.float32), atol=2e-6, rtol=1e-5
    )
