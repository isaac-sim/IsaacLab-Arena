...
def test_on_init_centers_geometry_when_child_wider_than_parent():
    """An oversized off-origin box is seeded with its geometry centered on the parent."""
    desk = DummyObject(
        name="desk",
    ...
    assert abs(x + 0.25 - center_x) < 1e-6
    assert abs(y + 0.25 - center_y) < 1e-6
