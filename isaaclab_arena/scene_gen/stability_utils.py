...
:mod:`isaaclab_arena.scene_gen.run_stability_check` composes these
...
def get_rigid_pose(env, name: str, env_id: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    pos = obj.data.root_pos_w[env_id, :3].clone().detach()
    quat = obj.data.root_quat_w[env_id, :4].clone().detach()
    return pos, quat
...
def get_rigid_velocity(env, name: str, env_id: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    obj = rigid_objects[name]
    lin = obj.data.root_lin_vel_w[env_id, :3].clone().detach()
    ang = obj.data.root_ang_vel_w[env_id, :3].clone().detach()
    return lin, ang
...
def compute_aabb_overlap_pairs(env, arena_env, names: list[str], env_id: int) -> list[dict]:
    ...
    for name in names:
        ...
        # ``min_point`` / ``max_point`` are plain ``tuple[float, float, float]``
        # on this branch; convert to a 1-D tensor for the corner builder.
        local_min = torch.tensor(tuple(local_bbox.min_point), dtype=torch.float32)
        local_max = torch.tensor(tuple(local_bbox.max_point), dtype=torch.float32)
