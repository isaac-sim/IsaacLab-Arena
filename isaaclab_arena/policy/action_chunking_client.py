...
class ActionChunkingClientSidePolicy(ClientSidePolicy):
    ...
    requested_action_mode: ActionMode = ActionMode.CHUNK
    ...
    def __init__(
        ...
        super().__init__(config=config, remote_config=remote_config)

        if self.action_mode is not ActionMode.CHUNK:
            raise ValueError(
                f"ActionChunkingClientSidePolicy expects CHUNK mode, "
    ...
    def _request_new_chunk(
        ...
        if not isinstance(resp, dict):
            raise TypeError(
                f"Expected dict from get_action, got {type(resp)!r}"
            )
        if "action" not in resp:
            raise KeyError(
                "Remote response does not contain key 'action' "
                "for ActionChunkingClientSidePolicy."
            )

        raw_chunk = resp["action"]
        if not isinstance(raw_chunk, torch.Tensor):
            raw_chunk = torch.tensor(
                raw_chunk, dtype=torch.float32, device=self._device
            )
        else:
        ...
        if raw_chunk.shape[0] != self._num_envs:
            raise ValueError(
                f"Expected batch size {self._num_envs}, got {raw_chunk.shape[0]}"
            )
        if raw_chunk.shape[1] < protocol.action_chunk_length:
            raise ValueError(
                f"Expected at least {protocol.action_chunk_length} "
                f"actions per chunk, got {raw_chunk.shape[1]}"
            )
        if raw_chunk.shape[2] != protocol.action_dim:
            raise ValueError(
                f"Expected action_dim {protocol.action_dim}, got {raw_chunk.shape[2]}"
            )

        return raw_chunk[:, : protocol.action_chunk_length, :]
    ...
    def get_action(
        ...
        if action.shape != (self._num_envs, protocol.action_dim):
            raise RuntimeError(
                ...
                f"expected {(self._num_envs, protocol.action_dim)}"
            )

        self._current_action_index += 1
        self._env_requires_new_chunk = (
            self._current_action_index >= protocol.action_chunk_length
        )
