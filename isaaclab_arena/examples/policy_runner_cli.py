...
#     Gr00tClosedloopPolicy.add_args_to_parser(args_parser)
...
def create_policy(args: argparse.Namespace) -> tuple[PolicyBase, int]:
    ...
    if args.policy_type == "replay":
        ...
        module_path, class_name = gr00t_policy_class_path.rsplit(".", 1)
        module = import_module(module_path)
        Gr00tClosedloopPolicy = getattr(module, class_name)

        # policy = Gr00tClosedloopPolicy(args.policy_config_yaml_path, num_envs=args.num_envs, device=args.policy_device)
        # policy = Gr00tClosedloopPolicy.from_args(args.policy_config_yaml_path, num_envs=args.num_envs, device=args.policy_device)
        policy = Gr00tClosedloopPolicy.from_args(args)
        num_steps = args.num_steps
    else:
