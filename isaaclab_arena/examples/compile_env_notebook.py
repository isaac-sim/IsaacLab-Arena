...
kitchen_counter_reference = ObjectReference(
    name="kitchen_counter",
    prim_path="{ENV_REGEX_NS}/lightwheel_robocasa_kitchen/counter_right_main_group",
    parent_asset=kitchen_background,
)

embodiment.set_initial_pose(Pose(position_xyz=(3.943, -1.069, 0.995), rotation_wxyz=(0.7071068, 0.0, 0.0, 0.7071068)))

# Note(cvolk): Currently only one Anchor object is supported.
# And as the ObjectReference objects are not allowed to be moved, they can only be used as anchors.
kitchen_counter_reference.add_relation(IsAnchor())
# Note(cvolk): This does not account the microwave or the knife block that are alson on the counter.
tomato_soup_can.add_relation(On(kitchen_counter_reference, clearance_m=0.02))
...
pick_and_place_task = PickAndPlaceTask(tomato_soup_can, refrigerator_shelf, kitchen_background)
...
# Note(cvolk): We need to pass the kitchen_counter_reference to the Scene's assets list for it to be picked up by the ObjectPlacer.
scene = Scene(
    assets=[
        kitchen_background,
        cracker_box,
...
for _ in tqdm.tqdm(range(NUM_STEPS)):
    with torch.inference_mode():
        ...
        if _ == STEPS_UNTIL_TELEPORT_VEGETABLE:
            ...
            tomato_soup_can.set_object_pose(
                env,
                env_ids=None,
