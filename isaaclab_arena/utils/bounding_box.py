...
class OrientedBoundingBox:
    ...
    def from_min_max(cls, min_point: Vector3, max_point: Vector3) -> "OrientedBoundingBox":
        ...
        assert torch.isfinite(minimum).all() and torch.isfinite(maximum).all(), "Box bounds must be finite."
        ...
        return cls(minimum * 0.5 + maximum * 0.5, maximum * 0.5 - minimum * 0.5, identity)
    ...
    def transformed(self, position_xyz: Vector3, rotation_xyzw: Quaternion) -> "OrientedBoundingBox":
        ...
        assert torch.isfinite(position).all(), "Position must be finite."
        rotation = _as_batched(rotation_xyzw, 4, self.center.device)
        norms = torch.linalg.vector_norm(rotation, dim=-1)
        assert torch.allclose(norms, torch.ones_like(norms), atol=1e-5, rtol=1e-5), "Rotation must be unit length."
    ...
    def get_bounds_along_axis(self, axis: Vector3) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        assert torch.isfinite(axis_tensor).all(), "Projection axes must be finite."
    ...
    def penetration(
        ...
        Args:
            ...
            clearance_m: Required separation along a SAT axis in metres. This is
                a conservative clearance test, not Euclidean surface distance.
            tie_break_sign: Directed escape sign for exactly concentric projections.

        Rows with separated enclosing AABBs are culled before SAT. Axis-aligned
        pairs use three world-axis projections; other pairs use all 15 SAT axes.
        """
        assert math.isfinite(clearance_m) and clearance_m >= 0.0, "Clearance must be finite and non-negative."
        center_a, extent_a, quat_a, center_b, extent_b, quat_b = self._paired(other)
        count = center_a.shape[0]
        if not isinstance(tie_break_sign, torch.Tensor):
        ...
        # Keep a differentiable zero even when every row is culled.
        projection = center_b - center_a
        distance = torch.where(projection == 0.0, tie_sign * projection, projection.abs())
        aabb_depth = (aabb_radius_a + aabb_radius_b + clearance_m - distance).amin(dim=-1).clamp_min(0.0)
        penetration = torch.where(broadphase, aabb_depth, torch.zeros_like(aabb_depth))
        axis_aligned = self.is_axis_aligned() & other.is_axis_aligned()
        active = torch.nonzero(broadphase & ~axis_aligned, as_tuple=True)[0]
        if active.numel() == 0:
            return penetration

        sat_depth = self._sat_penetration(
            projection[active],
            extent_a[active],
            extent_b[active],
            rotation_a[active],
            rotation_b[active],
            clearance_m,
            tie_sign.expand(count, 1)[active],
        )
        return penetration.index_copy(0, active, sat_depth)

    @staticmethod
    def _sat_penetration(
        ...
        clearance_m: float,
        tie_sign: torch.Tensor,
    ) -> torch.Tensor:
        """Return penetration for broadphase-positive pairs using 15 normalized axes."""
        axes_a = rotation_a.transpose(1, 2)
        axes_b = rotation_b.transpose(1, 2)
        cross_axes = torch.linalg.cross(axes_a.unsqueeze(2), axes_b.unsqueeze(1), dim=-1).flatten(1, 2)
        ...
        axes = axes * torch.where(dominant_component < 0.0, -1.0, 1.0)
        ...
        projection = torch.einsum("nkd,nd->nk", axes, center_delta)
        # abs() has zero derivative at zero. Select a directed derivative so
        # exactly concentric movable boxes receive an escape gradient.
        distance = torch.where(projection == 0.0, tie_sign * projection, projection.abs())
        depths = radius_a + radius_b + clearance_m - distance
        depths = torch.where(valid, depths, torch.full_like(depths, torch.inf))
        return depths.amin(dim=-1).clamp_min(0.0)
...
def get_random_pose_within_bounding_box(bbox: OrientedBoundingBox, seed: int | None = None) -> Pose:
    ...
    generator = None if seed is None else torch.Generator(device=bbox.center.device).manual_seed(seed)
    local_position = (2.0 * torch.rand(3, device=bbox.center.device, generator=generator) - 1.0) * bbox.half_extents[0]
