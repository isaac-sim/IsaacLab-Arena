...
def build_and_run_experiment(
    experiment_plan: ArenaExperimentPlan,
    output_dir: str | Path,
    video_cfg: VideoRecordingCfg | None = None,
    fallback_num_steps: int | None = None,
) -> ArenaExperimentResult:
    """Build and run one resolved experiment plan, then return its result."""
    cfg = experiment_plan.experiment_cfg
    ...
    for rebuild_index, num_episodes in enumerate(episodes_per_rebuild):
        ...
        try:
            ...
            env = _build_environment(cfg, rebuild_video_cfg.render_mode, experiment_plan.arena_builder_factory)
