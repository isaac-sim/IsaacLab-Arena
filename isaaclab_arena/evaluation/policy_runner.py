...
def main():
    """Script to run an IsaacLab Arena environment with a policy. Use --distributed with torchrun for one process per GPU."""
    args_parser = get_isaaclab_arena_cli_parser()
    # We do this as the parser is shared between the example environment and policy runner
    args_cli, unknown = args_parser.parse_known_args()

    # When --distributed: init process group and set cuda device; AppLauncher will launch one sim app per GPU (cuda:LOCAL_RANK)
    rank, world_size = 0, 1
    if getattr(args_cli, "distributed", False):
        rank, world_size = setup_distributed()
        print(f"Rank {rank}/{world_size}: one Isaac Lab instance per process on cuda:{rank}")

    # Start the simulation app (each process gets one sim on cuda:local_rank when --distributed)
