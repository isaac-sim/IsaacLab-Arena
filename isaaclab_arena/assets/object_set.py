...
class RigidObjectSet(Object):
    ...
    def get_bounding_box(self) -> OrientedBoundingBox:
        ...
        def height(bbox: OrientedBoundingBox) -> float:
            lower, upper = bbox.get_bounds_along_axis((0.0, 0.0, 1.0))
            return (upper[0] - lower[0]).item()

        return max((obj.get_bounding_box() for obj in self.objects), key=height)
