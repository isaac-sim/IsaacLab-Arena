...
For collision experiments we support:
  - Explicit colliders on the referenced visual USD.
  - A legacy single collision overlay.
  - Split floor / obstacle collision overlays built from a cleaner raw mesh.
"""
...
def _get_collision_overlay_specs(cfg) -> list[tuple[str, str, str]]:
    """Return collision overlay specs as ``(prim_name, usd_path, label)`` tuples."""
    combined_overlay = getattr(cfg, "collision_overlay_usd_path", None)
    floor_overlay = getattr(cfg, "floor_collision_usd_path", None)
    obstacle_overlay = getattr(cfg, "obstacle_collision_usd_path", None)

    if combined_overlay and (floor_overlay or obstacle_overlay):
        raise ValueError(
            "Legacy combined collision overlay cannot be used together with split "
            "floor / obstacle collision overlays."
        )

    specs: list[tuple[str, str, str]] = []
    if combined_overlay:
        specs.append(("collision", combined_overlay, "combined"))
    if floor_overlay:
        specs.append(("collisionFloor", floor_overlay, "floor"))
    if obstacle_overlay:
        specs.append(("collisionObstacle", obstacle_overlay, "obstacle"))
    return specs
...
def _spawn_matterport_with_ground(prim_path, cfg, *args, **kwargs):
    ...
    collision_overlay_stats: list[str] = []
    for overlay_prim_name, overlay_usd_path, overlay_label in _get_collision_overlay_specs(cfg):
        collision_overlay_mesh_count = _spawn_collision_overlay(
            prim_path=prim_path,
            stage=prim.GetStage(),
            usd_path=overlay_usd_path,
            physics_material_path=mat_path,
            approximation=mesh_collider_approximation,
            overlay_prim_name=overlay_prim_name,
        )
        collision_overlay_stats.append(
            f"{overlay_label}-overlay({collision_overlay_mesh_count}, {mesh_collider_approximation})"
        )
    ...
    if collision_overlay_stats:
        collider_str += " + " + " + ".join(collision_overlay_stats)
...
class MatterportBackground(LibraryBackground):
    ...
    def __init__(
        ...
        floor_collision_usd_path: str | None = None,
        obstacle_collision_usd_path: str | None = None,
    ):
        if collision_overlay_usd_path and (floor_collision_usd_path or obstacle_collision_usd_path):
            raise ValueError(
                "Use either the legacy combined collision overlay or the split floor / "
                "obstacle overlays, not both."
            )
        ...
        self.floor_collision_usd_path = floor_collision_usd_path
        self.obstacle_collision_usd_path = obstacle_collision_usd_path
    ...
    def _generate_base_cfg(self) -> AssetBaseCfg:
        ...
        object_cfg.spawn.floor_collision_usd_path = self.floor_collision_usd_path
        object_cfg.spawn.obstacle_collision_usd_path = self.obstacle_collision_usd_path
        return object_cfg
