...
class ObjectPlacer:
    ...
    def place(
        ...
        failed_envs = [i for i, result in enumerate(results_per_env) if not result.success]
        if failed_envs and not self.params.allow_best_loss_fallbacks:
            raise RuntimeError(f"No valid placement for envs {failed_envs} after {max_attempts} attempts.")
    ...
    def _place_ranked(
        ...
        assert candidates_per_env > 0, "max_placement_attempts must be positive."
        collision_objects = collision_objects or []
    ...
    def _sample_axis_position(
        ...
        align the child and parent centers as a stable seed.
    ...
    def _run_inexpensive_checks(
        ...
        for validator in self._validators:
            if not validator.run_after_inexpensive_checks:
                ...
                assert (
                    len(layout_pass_verdicts_by_check[validator.check]) == num_candidates
                ), f"Validator '{validator.check}' must return one verdict per candidate."
