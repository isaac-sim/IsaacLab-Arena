...
class RelationLossStrategy(ABC):
    ...
    def compute_loss(
        ...
        Args:
            ...
            child_pos: Child object position (N, 3) in world coords.
            child_bbox: Child object local bounding box (N=1).
            parent_world_bbox: Parent bounding box in world coordinates (N=1, broadcasts with child_pos).
...
class NextToLossStrategy(RelationLossStrategy):
    ...
    def compute_loss(
        ...
        parent_edge = parent_world_bbox.max_point[:, ax] if cfg.direction == Direction.POSITIVE else parent_world_bbox.min_point[:, ax]
        child_offset = child_bbox.min_point[0, ax] if cfg.direction == Direction.POSITIVE else child_bbox.max_point[0, ax]
        ...
        parent_band_min = parent_world_bbox.min_point[:, band_ax]
        parent_band_max = parent_world_bbox.max_point[:, band_ax]
