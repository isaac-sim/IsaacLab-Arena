...
def next_to_violations(
    ...
    if cfg.direction == Direction.POSITIVE:
        parent_edge = parent_max[:, cfg.primary_axis]
        child_offset = child_min[:, cfg.primary_axis]
        penalty_side = "less"
    else:
        parent_edge = parent_min[:, cfg.primary_axis]
        child_offset = child_max[:, cfg.primary_axis]
...
def not_next_to_violations(
    ...
    valid_band_min = parent_min[:, cfg.band_axis] - child_min[:, cfg.band_axis]
    valid_band_max = parent_max[:, cfg.band_axis] - child_max[:, cfg.band_axis]
...
class OnLossStrategy(RelationLossStrategy):
    ...
    def compute_loss(
        ...
        valid_x_min = parent_min[:, 0] + m - child_min[:, 0]
        valid_x_max = parent_max[:, 0] - m - child_max[:, 0]
        valid_y_min = parent_min[:, 1] + m - child_min[:, 1]
        valid_y_max = parent_max[:, 1] - m - child_max[:, 1]
        ...
        target_z = parent_max[:, 2] + relation.clearance_m - child_min[:, 2]
