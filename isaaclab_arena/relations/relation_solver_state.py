...
class RelationSolverState:
    ...
    def optimizable_positions(self) -> torch.Tensor | None:
        """Tensor of optimizable positions: [N_opt, 3] (single) or [num_envs, N_opt, 3] (batched)."""
    ...
    def get_position(self, obj: Object | ObjectReference, env_index: int | None = None) -> torch.Tensor:
        ...
        Args:
            ...
            env_index: In batched mode, the env index (0..num_envs-1). When None in batched mode,
                returns positions for all envs (num_envs, 3) for batched loss. When None in
                single-env mode, returns (3,).

        Returns:
            Position tensor (3,) in single-env, or (num_envs, 3) when batched and env_index None,
            or (3,) when batched and env_index is set.
        """
    ...
    def get_all_positions_snapshot(self) -> list[tuple[float, float, float]]:
        """Get detached copy of all positions for history tracking (single-env or env 0 in batched)."""
        if self._num_envs > 1:
    ...
    def get_final_positions_dict(self) -> dict[Object | ObjectReference, tuple[float, float, float]]:
        """Get final positions as a dictionary (single-env or env 0 in batched)."""
    ...
    def get_final_positions_per_env(self) -> list[dict[Object | ObjectReference, tuple[float, float, float]]]:
        """In batched mode, return one position dict per env. Single-env returns [get_final_positions_dict()]."""
        if self._num_envs <= 1:
