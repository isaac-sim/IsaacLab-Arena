...
class RelationSolverState:
    ...
    def __init__(
        ...
        assert all_positions.shape == (self._batch_size, len(objects), 3), "Positions must have shape (N, O, 3)."
        assert torch.isfinite(all_positions).all(), "Initial positions must be finite."
        ...
        self._base_bboxes: dict[PlaceableAsset, OrientedBoundingBox] = {}
        for obj in objects:
            bbox = env_bboxes[obj] if env_bboxes is not None and obj in env_bboxes else obj.get_bounding_box()
            assert bbox.num_envs in (
                1,
                self._batch_size,
            ), f"Bounding boxes for '{obj.name}' must have N=1 or N={self._batch_size}, got {bbox.num_envs}."
            self._base_bboxes[obj] = bbox.to(self._device)
        rotations = [{} for _ in range(self._batch_size)] if rotations is None else rotations
        assert (
        ...
        # Candidate geometry is constant unless FaceTo derives rotation from positions.
        self._fixed_candidate_bboxes = {
            obj: self._base_bboxes[obj].rotated_by_quat_unchecked(candidate_rotations[obj])
            for obj in self._optimizable_objects
            if get_relation(obj, FaceTo) is None
        }
        ...
        for obj in self._anchor_objects:
            ...
            self._fixed_obstacle_world_bboxes[obj] = self._base_bboxes[obj].transformed(
    ...
    def get_final_positions(self) -> list[dict[PlaceableAsset, tuple[float, float, float]]]:
        ...
        assert torch.isfinite(full).all(), "Relation solving produced non-finite positions."
