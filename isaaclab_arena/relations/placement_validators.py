...
class PlacementValidator(ABC):
    ...
    def is_available(cls, params: ObjectPlacerParams) -> bool:
        """Whether this check can run with the supplied placement parameters."""
        return True
    ...
    def validate_batch(
        ...
        Args:
            ...
            rotations: Environment-local xyzw rotations per movable object.
...
def build_validators(
    ...
    Explicitly enabled or required checks must be registered and available.
    Automatic discovery skips unavailable checks.
    ...
    requested = (params.enabled_checks or set()) | (params.required_checks or set())
    unknown = requested - set(registered_checks)
    assert not unknown, f"Unknown placement checks: {sorted(unknown)}."
    enabled_checks = params.enabled_checks
    if enabled_checks is not None:
        disabled = (params.required_checks or set()) - enabled_checks
        assert not disabled, f"Required placement checks are disabled: {sorted(disabled)}."
    ...
    for check in registered_checks:
        ...
        available = validator_cls.is_available(params)
        assert available or check not in requested, f"Requested placement check '{check}' is unavailable."
        if available:
...
def _candidate_local_bbox(
    ...
    """Return candidate-oriented local geometry on the validation device."""
    bbox = bbox.to("cpu")
    if obj.is_anchor:
        return bbox
...
def _candidate_world_bbox(
    ...
    """Return candidate or fixed-anchor world geometry on the validation device."""
    bbox = bbox.to("cpu")
    if obj.is_anchor:
    ...
    def _validate(
        ...
        for obj in positions:
            for rel in obj.get_relations():
                ...
                child_min, child_max = child_world.get_axis_aligned_bounds()
                parent_min, parent_max = parent_world.get_axis_aligned_bounds()
                child_x_min, child_y_min, child_z_min = child_min.unbind(dim=1)
                child_x_max, child_y_max, _ = child_max.unbind(dim=1)
                parent_x_min, parent_y_min, _ = parent_min.unbind(dim=1)
                parent_x_max, parent_y_max, parent_z_max = parent_max.unbind(dim=1)
                m = rel.edge_margin_m
                if (
...
class FaceToValidator(PlacementValidator):
    """A check that each FaceTo rotation points toward its target in the XY plane."""
    ...
    def _validate(
        ...
        """Check target direction and its corresponding quaternion."""
        for obj in positions:
            ...
            yaws, direction_is_defined = yaw_toward_positions(subject_position, target_position)
            ...
            if rotations is None or obj not in rotations:
                ...
                return False
            half_yaw = yaws[0].item() * 0.5
            expected = torch.tensor([0.0, 0.0, math.sin(half_yaw), math.cos(half_yaw)])
            rotation = torch.tensor(rotations[obj], dtype=expected.dtype)
            if rotation.shape != (4,) or not (
                torch.allclose(rotation, expected, atol=1e-5, rtol=0)
                or torch.allclose(rotation, -expected, atol=1e-5, rtol=0)
            ):
                return False
        return True
...
class NoOverlapValidator(PlacementValidator):
    """An overlap check for placed boxes, collision meshes, and fixed obstacles."""
        ...
        for i in range(len(objects)):
            for j in range(i + 1, len(objects)):
                ...
                if (id(a), id(b)) in on_pairs:
                    continue
                # Fixed BBOX obstacles keep the solver's exact OBB check.
                fixed = a if a in anchors else b if b in anchors else None
                if (
                    mesh_manager is not None
                    and fixed is not None
                    and (
                        not object_uses_mesh_collision(fixed, default_collision_mode)
                        or mesh_manager.get_collision_mesh(fixed) is None
                    )
                ):
                    yield a, b
                    continue
    ...
    def _validate_no_overlap(
        ...
        background_worlds = [(bg, bg.get_world_bounding_box().to("cpu")) for bg in collision_objects]
        mesh_manager = self._get_cpu_mesh_manager() if skip_mesh_pairs else None
        default_collision_mode = self._params.solver_params.collision_mode
        for obj in positions:
            if obj in anchors:
    ...
    def _validate_no_overlap_mesh(
        ...
        for a, b in self._non_skip_pairs(positions):
            ...
            if (a.is_anchor and a_mesh is None) or (b.is_anchor and b_mesh is None):
                continue
            ...
            if not a.is_anchor and self._spheres_penetrate_mesh(
                a,
                a_collision_mesh,
                ...
                return False
            if not b.is_anchor and self._spheres_penetrate_mesh(
                b,
                b_collision_mesh,
                b if b_mesh is not None else None,
                b_pos,
