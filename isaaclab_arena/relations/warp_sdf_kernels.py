...
class _MeshSDFFunction(torch.autograd.Function):
    ...
    Backward: analytical gradient ``grad(sdf) = normalize(point - closest)``,
    which is the outward unit normal of the SDF iso-surface.
    """
    ...
    def forward(
        ...
        ctx.save_for_backward(query_points_torch, closest_torch)
        return sdf_torch

    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        query_pts, closest_pts = ctx.saved_tensors

        direction = query_pts - closest_pts
        dir_len = direction.norm(dim=-1, keepdim=True).clamp(min=1e-6)
        grad_sdf_wrt_pts = direction / dir_len
