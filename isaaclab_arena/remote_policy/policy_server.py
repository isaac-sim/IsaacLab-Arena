...
class PolicyServer:
    ...
    def _handle_get_init_info(
        ...
        if not isinstance(resp, dict):
            raise TypeError(
                f"Policy.get_init_info() must return dict, got {type(resp)!r}"
            )
        return resp
    ...
    def _handle_get_action(
        ...
        **_: Any,
        ...
        if not isinstance(action, dict):
            raise TypeError(
                f"Policy.get_action() must return (dict, dict), "
                f"got action type={type(action)!r}"
            )
        if not isinstance(info, dict):
            raise TypeError(
                f"Policy.get_action() must return (dict, dict), "
                f"got info type={type(info)!r}"
            )
        ...
        if any(k in merged for k in info.keys()):
            raise ValueError(
                f"Policy info keys conflict with action keys: "
                f"{set(merged.keys()) & set(info.keys())}"
            )
        merged.update(info)
