...
class ServerTransport(ABC):
    ...
    def recv_with_timeout(self, timeout_ms: int) -> tuple[bytes, bytes]:
        """Receive with a custom timeout (for batch collection).

        Default implementation delegates to ``recv()``; subclasses should
        override to temporarily adjust the socket timeout.

        Raises:
            TimeoutError: if no message arrives within *timeout_ms*.
        """
        return self.recv()
