...
class WebsocketClientPolicy(websocket_client_policy.WebsocketClientPolicy):
    ...
    The Cosmos RoboLab policy server serves over openpi's ``WebsocketPolicyServer``
    (msgpack+NumPy protocol), so the same client speaks to it; this override only
    exposes the ping interval/timeout that upstream's client hard-codes.
    """
