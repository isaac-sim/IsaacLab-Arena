...
```bash
python isaac_arena/examples/zero_action_runner.py kitchen_pick_and_place --object cracker_box --embodiment gr1
```

or

```bash
python isaac_arena/examples/zero_action_runner.py gr1_open_microwave --object tomato_soup_can
```

**NOTE:** CLI arguments are sensitive to order. They must appear in the following order:

```
python isaac_arena/examples/zero_action_runner.py <--global flags> <example app name> <--app specific flags>
```
