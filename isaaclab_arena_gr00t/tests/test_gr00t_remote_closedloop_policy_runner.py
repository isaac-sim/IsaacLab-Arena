...
SINGLE_ENV_COUNT = 1
MULTI_ENV_COUNT = 3
ARENA_EXPERIMENT_RUNNER_NUM_STEPS = 2
...
def _write_eval_jobs_config(tmp_path, remote_host: str, remote_port: int) -> str:
    jobs = [
        {
            ...
            "num_steps": ARENA_EXPERIMENT_RUNNER_NUM_STEPS,
    ...
    config_path = tmp_path / "test_gr00t_remote_closedloop_arena_experiment_runner.json"
...
def test_arena_experiment_runner_with_gr00t_remote_closedloop_policy(tmp_path):
    """Run the Arena Experiment Runner with the GR00T remote closed-loop policy."""
    remote_host, remote_port, timeout_sec = _get_gr00t_remote_server()
    config_path = _write_eval_jobs_config(tmp_path, remote_host, remote_port)

    args = [
        TestConstants.python_path,
        f"{TestConstants.evaluation_dir}/arena_experiment_runner.py",
        "--eval_jobs_config",
        config_path,
        "--headless",
    ]
