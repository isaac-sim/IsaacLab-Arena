...
def load_gr00t_modality_config_from_file(modality_config_path: str | Path | None, embodiment_tag: str):
    ...
    Args:
        ...
        embodiment_tag: Embodiment tag naming the modality configuration to load, as spelled in GR00T's `groot.core.data.schema.embodiment_tags.EmbodimentTag`.
    Returns:
        modality_configs: Modality configurations
    """
    from groot.core.data.schema.embodiment_tags import EmbodimentTag
    from groot.core.training.configs.data.embodiment_configs import MODALITY_CONFIGS
    ...
    if modality_config_path and embodiment_tag_key not in MODALITY_CONFIGS:
        ...
        # registering entries into MODALITY_CONFIGS. GR00T N2 removed the
        # gr00t.experiment.launch_finetune helper this previously preferred, so load the
        # module directly.
