...
class ReachabilityValidator(PlacementValidator):
    ...
    def validate_batch(
        ...
        return [self._validate(positions[i], orientations[i], batch_index=i) for i in range(len(positions))]
    ...
    def _validate(
        ...
        if self._rerun_layer is not None:
            self._rerun_layer.log_candidate(
                candidate_index=self._visualizer.candidate_index_for_batch_index(batch_index),
