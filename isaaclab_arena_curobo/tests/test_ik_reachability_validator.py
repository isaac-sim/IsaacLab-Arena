...
"""Reachability orchestration tests with mocked IK and grasp calls.

Validator tests require cuRobo imports. OBB cuboid conversion is tested
separately on CPU and CUDA without cuRobo.
"""
...
def test_collision_cuboid_composes_intrinsic_bbox_rotation(device):
    ...
    cuboid = get_obb_collision_cuboid_for_object(
        ...
        pos_w=(1.0, 2.0, 3.0),
        quat_w_xyzw=(0.0, 2**-0.5, 0.0, 2**-0.5),
    )

    assert cuboid.pose_W_O.position_xyz == pytest.approx((1.0, 2.0, 2.75))
    assert cuboid.pose_W_O.rotation_xyzw == pytest.approx((0.5, 0.5, 0.5, 0.5))
    assert cuboid.dims_xyz == pytest.approx((0.4, 0.2, 0.1))
