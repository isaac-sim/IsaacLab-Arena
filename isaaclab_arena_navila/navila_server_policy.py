...
class NaVilaServerPolicy(ServerSidePolicy):
    ...
    def get_action(
        ...
        if rgb_np is not None:
            ...
            # Keep FULL history (no truncation) — NaVILA samples uniformly
            # from the entire episode history to give the VLM global context.
    ...
    def _run_vlm_inference(self) -> str:
        ...
        if num_images < n:
            # Pad by repeating the first real image to give the VLM a
            # coherent visual context in early queries (black padding causes
            # the VLM to spin aimlessly).
