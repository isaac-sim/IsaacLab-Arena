...
"""Standalone diagnostic that proves the mesh non-collision loss works.

Runs the RelationSolver on trimesh-backed DummyObjects (no Isaac Sim) and
visualises meshes, SDF-coloured query points, the loss curve, and an
optimization animation. Shows meshes stay clear where AABBs overlap and
the solver separates overlapping meshes.
...
def _extract_trimesh_from_usd(usd_path: str, scale: tuple[float, float, float] = (1.0, 1.0, 1.0)) -> trimesh.Trimesh:
    """Load all USD mesh geometry into one trimesh, baking per-prim world transforms and scale.

    Requires pxr (OpenUSD), available in the Isaac Sim Python env.
    """
...
def make_usd_object(name: str) -> DummyObject:
    """Create a DummyObject from a cached USD asset under /tmp/Assets/.

    The asset must have been downloaded during a prior simulation run.
    Supports: mustard_bottle, cracker_box, power_drill, tomato_soup_can, sugar_box.
    """
...
def _compute_sdf_for_pair(
    ...
    assert child_mesh is not None and parent_mesh is not None, "SDF pair needs collision meshes on both objects."

    child_verts = torch.tensor(np.asarray(child_mesh.vertices), dtype=torch.float32, device=device)
    parent_verts = torch.tensor(np.asarray(parent_mesh.vertices), dtype=torch.float32, device=device)

    parent_wp = mgr.get_warp_mesh(parent_mesh, parent_obj)
    child_wp = mgr.get_warp_mesh(child_mesh, child_obj)

    # Query each object's surface points in the other's local frame.
    with torch.no_grad():
        sdf_a = mesh_sdf(child_verts + c_pos - p_pos, parent_wp)
        sdf_b = mesh_sdf(parent_verts + p_pos - c_pos, child_wp)

    child_query_world = (child_verts + c_pos).cpu().numpy()
    parent_query_world = (parent_verts + p_pos).cpu().numpy()
    return child_query_world, sdf_a.cpu().numpy(), parent_query_world, sdf_b.cpu().numpy()
...
def run_solver_with_mesh_viz():
    ...
    fig1 = _build_scene_figure(
        all_objects, positions_dict, title="Final Scene: Meshes + Bounding Spheres", show_spheres=True
    )
...
def _build_scene_figure(
    ...
    Args:
        show_spheres: If True, overlay bounding-sphere decompositions on each
            non-anchor object (semi-transparent icospheres).
    """
    fig = go.Figure()
    ...
    for idx, obj in enumerate(objects):
        ...
        if show_spheres and mesh is not None and obj.name in non_anchor_names:
            spheres_np = mgr.get_query_spheres(mesh, obj).numpy()
            for trace in RelationSolverVisualizer.create_sphere_traces(
                spheres_np, position=pos, color=sphere_color, name_prefix=f"{obj.name} spheres", opacity=0.2
            ):
                fig.add_trace(trace)
...
def run_real_objects_demo():
    ...
    fig1 = _build_scene_figure(
        all_objects, positions_dict, title="Real YCB Objects: Solved Positions + Spheres", show_spheres=True
    )
