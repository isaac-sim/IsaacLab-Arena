...
"""Configure and submit a dummy gr00t-server OSMO workflow.

Runs ``echo "hello world"`` inside the GR00T server image to verify the image
...
from workflows.gr00t_server_workflow import Gr00tServerWorkflow
from workflows.utils.workflow_types import WorkflowType


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Configure and submit a dummy gr00t-server OSMO workflow.",
    ...
    workflow.add_argument("--workflow_name", default="arena-gr00t-server", help="OSMO workflow name")
...
def main(cli_args: list[str] | None = None) -> int:
    ...
    workflow = Gr00tServerWorkflow(
        workflow_type=WorkflowType.GR00T_SERVER,
        workflow_args=args,
        task_args=args,
    )
