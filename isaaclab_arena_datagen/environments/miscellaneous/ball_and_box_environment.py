...
class BallAndBoxEnvironment(ExampleEnvironmentBase):
    """Sphere and cracker box with gravity disabled and opposite vertical velocities."""

    scene_metadata = make_reference_metadata("ball_and_box", num_objects=2)
    ...
    def get_env(self, _args_cli: argparse.Namespace) -> Any:
        ...
        sphere.set_initial_velocity(Velocity(linear_xyz=(0.0, 0.0, -0.5)))
        ...
        cracker_box.set_initial_velocity(Velocity(linear_xyz=(0.0, 0.0, 0.5)))
        # IsaacLab leaves `rigid_props=None` on the spawn cfg, so default-
        # construct it before flipping `disable_gravity`.
        if cracker_box.object_cfg.spawn.rigid_props is None:
            cracker_box.object_cfg.spawn.rigid_props = sim_utils.RigidBodyPropertiesCfg()
        cracker_box.object_cfg.spawn.rigid_props.disable_gravity = True
        ...
        scene = Scene(assets=[sphere, cracker_box, light, distant_light])
    ...
    def get_default_cameras(_num_steps: int) -> list[CameraViewTrajectory]:
        ...
        return [
            CameraViewTrajectory(
                position=(0.0, -0.587, 0.9),
                target=(0.466, -0.587, 0.277),
    ...
    def add_cli_args(parser: argparse.ArgumentParser) -> None:
        """Add ball-and-box-specific CLI arguments to the parser."""
