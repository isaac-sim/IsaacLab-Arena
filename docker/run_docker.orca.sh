...
# Check if OMNI_PASS is set
if [ -z "$OMNI_PASS" ]; then
    echo "OMNI_PASS is not set. Please set the OMNI_PASS environment variable to access nucleus dev server."
    exit 1
fi
...
if [ "$( docker container inspect -f '{{.State.Running}}' $DOCKER_IMAGE_NAME'-'$DOCKER_VERSION_TAG 2>/dev/null)" = "true" ]; then
  ...
  docker exec -it $DOCKER_IMAGE_NAME-$DOCKER_VERSION_TAG su $(id -un)
    ...
    DOCKER_RUN_ARGS=("--name" "$DOCKER_IMAGE_NAME-$DOCKER_VERSION_TAG"
                    ...
                    "--env" "DATASET_DIR=/datasets/isaaclab_arena/locomanipulation_tutorial"
                    "--env" "MODELS_DIR=/models/isaaclab_arena/locomanipulation_tutorial"
                    "--env" "DISPLAY"
                    "--env" "ACCEPT_EULA=Y"
