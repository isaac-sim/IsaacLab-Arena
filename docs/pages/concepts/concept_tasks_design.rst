...
``SequentialTaskBase`` takes a list of ``TaskBase`` instances and automoatically composes them into a single task.
...
.. code-block:: python
    ...
    pick_object = asset_registry.get_asset_by_name("mustard_bottle")()
    destination = ObjectReference("refrigerator_shelf", parent_asset=kitchen_background)
    pick_and_place_tas = PickAndPlaceTask(pick_object, destination, kitchen)

    openable_object = OpenableObjectReference("refrigerator", parent_asset=kitchen_background, openable_joint_name="fridge_door_joint")
    close_door_task = CloseDoorTask(openable_object, closedness_threshold=0.10)

    sequential_task = SequentialTaskBase(subtasks=[pick_and_place_tas, close_door_task])
