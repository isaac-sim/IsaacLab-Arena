...
``environment_generation_runner.py`` is the scripted entry point for inspecting
...
.. list-table::
   ...
   * - ``resolve``
     ...
     - **Prompt** → **agent** → **spec**.
   * - ``build``
     - Loads ``--env_graph_spec_yaml``, converts it to an Arena environment,
...
.. code-block:: bash
   ...
   python isaaclab_arena_examples/agentic_environment_generation/environment_generation_runner.py \
      --mode build \
      --env_graph_spec_yaml isaaclab_arena_environments/agent_generated/<env_name>.yaml \
      --headless
...
.. list-table::
   ...
   * - Option
     - Purpose
     - System stage
   * - ``--mode {full,resolve,build,schema,catalog}``
...
.. list-table::
   ...
   * - Option
     - Purpose
     - System stage
   * - ``--env_graph_spec_yaml PATH``
     - Input graph-spec YAML. Required by ``--mode build``.
     - Spec
   * - ``--headless``
