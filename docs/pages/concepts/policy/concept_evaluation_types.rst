...
**policy runner** (single or multi-GPU) and an **Arena Experiment Runner** for
...
.. _sequential-batch-arena-experiment-runner:

2. Arena Experiment Runner — batch jobs
--------------------------------------------

The **Arena Experiment Runner** (``isaaclab_arena/evaluation/arena_experiment_runner.py``)
...
- **Distributed evaluation is not supported**: the Arena Experiment Runner
...
.. todo::
    ...
    Experiment with distributed evaluation in the Arena Experiment Runner.
...
**Running the Arena Experiment Runner**

.. code-block:: bash

   python isaaclab_arena/evaluation/arena_experiment_runner.py \
     --viz kit \
     --eval_jobs_config path/to/eval_jobs_config.json \
     --num_steps 1000

If any job needs cameras, set ``enable_cameras: true`` in that job’s
``arena_env_args``; the Arena Experiment Runner automatically enables camera support if any job requires it.
