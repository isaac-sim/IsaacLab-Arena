...
When running batches of jobs with ``arena_experiment_runner.py``, variations are configured per job via a
...
``--list-variations`` works with ``arena_experiment_runner.py`` too, printing the variations catalogue for
