...
Sharing variation values across runs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Use the top-level ``shared`` mapping when several runs should use the same values. Each run that
should receive those values references them explicitly. For example, this abbreviated config keeps
the camera displacement disabled for normal evaluations while sharing the same HDR and camera
settings across both runs:
...
.. code-block:: yaml
   ...
   runs:
     banana_in_bowl_pi0:
       # environment, policy, and other run fields
...
Enable the camera displacement for every run that references ``shared.variations`` with one
...
.. code-block:: bash
   ...
   python isaaclab_arena/evaluation/experiment_runner.py \
     --headless \
     --output_base_dir /eval/robolab_camera_sensitivity \
     --experiment_config experiment.yaml \
     shared.variations.droid_abs_joint_pos.camera_extrinsics_wrist_camera.enabled=true

When submitting the same config through OSMO, prefix the shared path with ``experiment_cfg``:
...
Values in ``shared`` apply only where a run references them; they do not limit which fields can be
