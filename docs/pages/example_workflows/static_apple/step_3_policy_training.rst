...
*standalone Isaac-GR00T checkout* (``$ISAAC_GR00T_DIR`` from :doc:`index`) for fine-tuning. The
fine-tuned checkpoint is later served back to Arena over the server-client architecture in
:doc:`step_4_evaluation`.

The N1.7 fine-tune script lives in the standalone Isaac-GR00T repo, not in Arena's pinned
...
Inside the container, set the dataset directory:
...
.. dropdown:: Download Pre-Generated Dataset (Skip Teleoperation)
   ...
   Use these commands to download the pre-recorded static apple HDF5 dataset ready for
   LeRobot conversion, so that you can skip the teleoperation step.
...
.. caution::
   ...
   EGO RGB under ``observations/camera_obs/robot_head_cam_rgb`` (refer to ``pov_cam_name_sim`` in the
...
tutorial filename so you can use the default config unchanged.
...
parquet files with states and actions, MP4 camera recordings, and dataset metadata.

A config file at
``isaaclab_arena_gr00t/lerobot/config/g1_static_apple_config.yaml`` controls the converter.
...
``data_root`` and ``hdf5_name`` pointing at the static apple-to-plate dataset and the
...
.. note::
   ...
   that PinkIK produced during teleoperation. That is why this workflow records with
   ``g1_wbc_agile_pink`` and evaluates with ``g1_wbc_agile_joint``. The policy never sees the
...
Post-train the GR00T N1.7 policy on the task using the *standalone Isaac-GR00T checkout* from
:doc:`index` (referenced as ``$ISAAC_GR00T_DIR``). This step runs *outside the Arena container* so
GR00T's dependencies do not have to coexist with the Arena and Isaac Sim ones.

The GR00T N1.7 policy has three billion parameters, so post-training is an expensive operation.
The command below is the tested single-GPU configuration for this workflow.

Training takes approximately two to three hours for 20,000 steps on a single NVIDIA RTX 6000 Ada GPU.

Compute Requirements:

- **GPUs:** one NVIDIA RTX 6000 Ada or another GPU with at least 48 GB VRAM
...
- **Action Horizon:** 40 (must match the diffusion head value used at evaluation; refer to the note below)
...
Because fine-tuning runs outside the Arena Docker container, set ``DATASET_DIR`` and ``MODELS_DIR``
...
.. note::
   ...
   **N1.7 CLI compared to N1.6 CLI.** The N1.7 ``launch_finetune.py`` is built on `tyro
...
.. note::
   ...
   layout (five state keys and seven action keys). This is the *same file* the server consumes at
...
If you have more powerful GPUs, refer to the
...
information on how to adjust the training configuration to your hardware. Fine-tune
the visual backbone, projector, and diffusion model for better results.


Recommendations for Fine-Tuning That Works with AGILE on Apple-to-Plate
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
whether the fine-tuned policy actually works in the AGILE-joint runtime:

#. **Record with AGILE, not HOMIE.** Keep the default ``--embodiment g1_wbc_agile_pink`` during
   teleoperation. AGILE's WBC is a single end-to-end velocity policy. HOMIE is a stand-and-walk pair.
...
#. **Pick** ``action_horizon`` **deliberately.** The default (40) gives an 800 ms inference chunk at
   ...
   apple-to-plate (~600-step episodes), 40 is a good default. You can go lower (for example, 20) for more
   ...
   server YAML in sync* (refer to the caution above).

If you adjust any of these and the resulting checkpoint behaves badly at evaluation, the most
common culprits, in order, are:

#. Too few or low-quality demonstrations.
#. Modality config or ``action_horizon`` mismatch between training and server YAML.
#. Recording with the wrong embodiment.
