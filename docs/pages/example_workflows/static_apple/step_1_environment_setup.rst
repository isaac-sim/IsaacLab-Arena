...
This page describes the environment used in this example workflow
and validates that you can load it in Isaac Lab.
...
The static apple-to-plate workflow ships its own environment,
``GalileoG1StaticPickAndPlaceEnvironment`` (registered as
``galileo_g1_static_pick_and_place``). It is a no-locomotion variant of the
loco-manipulation apple-to-plate environment that:

- Reuses the ``galileo_locomanip`` shelf scene and OpenXR retargeter
- Records with the ``g1_wbc_agile_pink`` embodiment by default
- Uses AGILE's single-policy lower-body backend, a better fit than HOMIE's
  stand-and-walk split because the task never walks
- Keeps the apple and plate on the same shelf, so the WBC only holds the
  standing pose

.. note::

   **Recording and evaluation embodiments.** Teleoperation in :doc:`step_2_teleoperation` uses the
   ...
   the same AGILE lower-body backend. The ``_joint`` twin bypasses PinkIK at inference because
   the policy is trained on the joint-space targets that PinkIK produced during recording. This
   matters for fine-tuning. Refer to :doc:`step_3_policy_training` for the rationale.
...
At a high level, defining an Arena environment means selecting assets, composing a scene, choosing
...
The production environment adds task-specific defaults around this core composition:

- Same-shelf poses for the apple and plate
- A short episode timeout
- An invisible shelf support surface
- Tuned contact and friction settings
- Cleanup of a few reused background props

Those details are handled inside the registered environment so workflow users can run it through the
CLI without editing the scene setup code.

Refer to :doc:`../../concepts/concept_overview`, :doc:`../../concepts/scene/index`, and
...
The tests run headless with cameras enabled and take around a
minute each. They are the fastest way to confirm the scene, task, and termination logic are
...
.. note::
   ...
   **First-run asset cache**: The Objaverse apple USD streams from an S3 staging bucket and PhysX
   ...
   and spawn correctly. If you see the apple fall through the shelf, re-run the command once.
