...
panorama and the ambient lighting. See :doc:`first_experiments/index` for an experiment where the HDR
...
It also attaches metrics (``SuccessRateMetric``, ``ObjectMovedRateMetric``) that ``arena_experiment_runner.py``
